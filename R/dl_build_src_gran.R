...
#' @param delete logical argument to over-ride deleting packages that 
#' are already built. This is useful for major R upgrades. Default
#' is \code{TRUE}.
#' @param use_GRAN logical argument to include GRAN in updates. Default is 
#' \code{TRUE}. Useful to use \code{FALSE} for major R upgrades.
#' @import httr 
#' @import utils
#' @import devtools
#' @importFrom miniCRAN pkgDep
#' @export
dl_build_src <- function(GRAN.dir = 'GRAN', lib=.libPaths()[1], delete = TRUE, use_GRAN = TRUE){
	...
	if(nrow(packages) > 0){
		...
		if(dir.exists(file.path(GRAN.dir, 'src'))){
			if(delete){
				...
				pkgs <- readRDS(file.path(src_dir,"PACKAGES.rds"))
				packages <- packages[which(!(sapply(strsplit(as.character(packages$package), split = "/"), function(x) x[2]) %in% pkgs[,1])),]
			}
		}else{
		...
		for(i in 1:nrow(packages)){
			...
			httr::GET(url, write_disk(file.path(scratch, 'package.zip'), overwrite=TRUE), timeout=600)
			...
			system(cmd)
			tools::write_PACKAGES(getwd(), type='source')
		...
		tools::write_PACKAGES(src_dir, type='source')
		...
		granbuild:::writeBuildList(src_dir)
	} else {
